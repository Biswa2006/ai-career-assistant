...
Provides a professional dark-themed chat UI with:
- Conversational chat interface with metadata badges per response
- Sidebar user profile configuration (name, skills, target role)
- Quick skill gap analysis button
- Session statistics and new conversation control
- Five pre-populated sample question shortcuts
"""
...
.meta-row { display: flex; gap: 0.5rem; flex-wrap: wrap; margin-top: 0.6rem; }
.badge {
    font-family: 'Space Mono', monospace;
    font-size: 0.65rem;
    padding: 2px 8px;
...
def init_session() -> None:
    """Initialises all required Streamlit session state keys on first load."""
    defaults = {
        "messages":          [],
...
_ROLE_OPTIONS = ["", "Data Analyst", "Data Scientist", "Software Engineer", "AI Engineer"]
...
with st.sidebar:
    ...
    api_key_input = st.text_input(
        ...
        help="Required for AI-generated answers. Obtain yours at platform.openai.com",
    )
    if api_key_input:
    ...
    if st.session_state.api_key_set:
        st.markdown(
            '<div style="color:#4ade80;font-size:0.75rem;">✅ API Key configured</div>',
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
            '<div style="color:#f59e0b;font-size:0.75rem;">'
            '⚠️ API Key not set — skill gap tool still works</div>',
            unsafe_allow_html=True,
        )
    ...
    # Determine current selectbox index safely
    current_role = st.session_state.target_role
    role_index = (
        _ROLE_OPTIONS.index(current_role)
        if current_role in _ROLE_OPTIONS else 0
    )
    ...
    if st.button("🔍 Run Skill Gap Analysis Now", use_container_width=True):
        if st.session_state.user_skills and st.session_state.target_role:
            with st.spinner("Analysing skill gap..."):
                result_text = skill_gap_analyzer(
                    st.session_state.user_skills,
            ...
            st.session_state.messages.append({
                "role":    "assistant",
                "content": result_text,
                "meta":    {"route": "tool", "faithfulness": 0.95, "sources": [], "eval_retries": 0},
            })
    ...
    faith_scores = st.session_state.avg_faithfulness
    ...
    <div class="info-box">
        ...
        <strong>LLM:</strong> OpenAI GPT-based models<br>
        <strong>Documents:</strong> 10 curated career knowledge documents<br>
        <strong>Memory:</strong> Thread-based sliding window (6 messages)
    </div>
    """, unsafe_allow_html=True)
...
def _render_bold(text: str) -> str:
    ...
    for i, part in enumerate(parts):
        result.append("<strong>" + part + "</strong>" if i % 2 == 1 else part)
    return "".join(result)
...
if not st.session_state.messages:
    ...
    <div class="empty-state">
        <div class="icon">🎓</div>
        <h3>Hello{greeting}! Ready to explore your career path?</h3>
        <p style="font-size:0.85rem; color:#475569; max-width:500px; margin:0 auto;">
            ...
            I can help with skill requirements, salaries, roadmaps,
            interview tips, and personalised skill gap analysis.
        </p>
    ...
    for msg in st.session_state.messages:
        if msg["role"] == "user":
            ...
            faith_class = (
                "badge-faith-high" if faithfulness >= 0.8 else
                "badge-faith-mid"  if faithfulness >= 0.6 else
                "badge-faith-low"
            )

            source_topics = (
                ", ".join(s.get("topic", "") for s in sources if s.get("topic"))
                or "general knowledge"
            )

            retry_badge = (
                f'<span class="badge badge-faith-mid">🔄 retried {eval_retries}x</span>'
                if eval_retries > 0 else ""
            )

            content = _render_bold(msg["content"]).replace("\n", "<br>")
...
# Use prefill from sidebar button if no direct input was provided
...
<div style="text-align:center; color:#1e293b; font-size:0.72rem;
    ...
    Agentic AI Career Decision Assistant &bull; LangGraph Architecture &bull;
    ChromaDB + all-MiniLM-L6-v2 &bull; Capstone Project 2025–26
</div>
""", unsafe_allow_html=True)
